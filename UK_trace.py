...
### England, Scotland, and Wales (minus two island constituencies)
...
ws.cell(row=num_rows+3,column=1).value='# of blocks'
ws.cell(row=num_rows+4,column=1).value=num_gray

### Now, do Northern Ireland

img=Image.open('./nireland.gif')
xsize=img.size[0]
ysize=img.size[1]
mode="RGB"
im=img.convert(mode)
...
if('Northern Ireland' in sheets):
    ws=wb.get_sheet_by_name('Northern Ireland')
else:
    ws=wb.create_sheet('Northern Ireland')
grayFill=PatternFill(start_color='FF808080',
                     end_color='FF808080',
                     fill_type='solid')
whiteFill=PatternFill(start_color='FFFFFFFF',
